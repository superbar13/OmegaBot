        ...
        if (interaction.options.getSubcommand() == 'set' || interaction.options.getSubcommand() == 'reset') {
                    ...
                    collector.on('collect', async m => {
                        ...
                        if (option.type == 'number' || option.type == 'string' || option.type == 'message') {
                            ...
                            if (channel) {
                                modifiedvalue = channel.id;
                                return collector.stop('success');
                            } else return collector.stop('failed');
                        } else if (option.type == 'role') {
                            // save the value
            ...
            if (interaction.options.getSubcommand() == 'reset') {
                ...
                if (updatestring) {
                    try {
                        await interaction.client.serversdb.bulkWrite([
                            interaction.client.bulkutility.unsetField({
                                'id': interaction.guild.id,
                            }, {
                                [updatestring]: '',
                            })
                ...
                function getOptionsInCat(tosearch, filtered, catObj, catSubObj) {
                    for (const [key, option] of Object.entries(tosearch)) {
                        if (['showedcategory', 'databasecategory'].includes(option.type) && option.showed == false) {
                            ...
                            if (!newtosearch.includes(option.displayname)) newtosearch.push(option.displayname);
                        }
                    }
                }
