...
module.exports = {
    ...
    run: async(client) => {
        ...
        client.on('messageCreate', async message => {
            ...
            if(command?.data?.dm_permission == false && (message.channel.type == 1 || message.channel.type == 3)){
                return message.reply({ embeds: [new EmbedBuilder().setTitle("Salon invalide").setDescription(`Vous ne pouvez pas exécuter cette commande en message privée. Réessayer depuis un serveur.`).setColor(client.embedcolor)] })
            }
