    ...
    run: async (client) => {
        ...
        client.guilds.cache.forEach(async guild => {
            ...
            if (info?.voiceconfig?.type == 'music') {
                if (!client.config.modules['music'].enabled) return console.log('[VOICE] Music module disabled, skipping...'.brightBlue);
                console.log(('[VOICE] Music was playing on ' + guild.name).brightBlue);
                client.modules.music.musicLoad(guild, info);
        ...
        client.on('voiceStateUpdate', async (oldState, newState) => {
            ...
            if (config.voiceconfig.type == 'radio') {
                ...
                client.modules.music.musicLoad(newState.guild, config);
            } else {
